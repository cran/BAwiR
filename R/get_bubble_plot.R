...
get_bubble_plot <- function(df_stats, player, descr_stats, size_text, size_text_x, size_legend){
  ...
  percs <- sapply(1:dim(df_stats1)[2], Anthropometry::percentilsArchetypoid, 
  ...
  gg <- ggplot(df_cp, aes(x = stat, y = outof4, col = descr, fill = outof4_f)) + 
    ...
    geom_hline(yintercept = seq(0, 4, by = 1), colour = "#949494", linewidth = 0.5, lty = 3) + #949494 is dark grey.
    geom_vline(xintercept = seq(0.5, nrow(df_cp), 1), colour = "#949494", linewidth = 0.4, lty = 1) +
    facet_wrap(~player) +
    coord_polar() + 
